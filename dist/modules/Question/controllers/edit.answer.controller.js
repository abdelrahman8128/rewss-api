...
exports.editAnswerController = (0, express_async_handler_1.default)(async (req, res) => {
    const service = new question_service_1.default();
    const { questionId } = req.params;
    const { content } = req.body;
    try {
        const updated = await service.editAnswer(questionId, content, {
            _id: req.user._id,
            role: req.user.role,
        });
        res
            .status(http_status_codes_1.StatusCodes.OK)
            .json({ message: "Answer updated", data: updated });
    }
    catch (error) {
